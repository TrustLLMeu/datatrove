...
class C4QualityFilter(BaseFilter):
    ...
    def __init__(
        ...
        language: str = Languages.english, # used for sentence tokenization. Mostly similar across across TrustLLM languages.
    ):
    ...
    def filter(self, doc: Document) -> bool | tuple[bool, str]:
        """
        C4-style line filtering does not play nice with structured output that treats \n\n as paragraphs (e.g. markdown).
        Let's see if it works when we split on those \n\n instead.
        """
        ...
        for line in lines:
            ...
            if self.min_num_sentences != -1:
                num_sentences += len(self.tokenizer.sent_tokenize(line))
